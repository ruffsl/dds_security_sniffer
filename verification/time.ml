...
let time_to_int (year: int) (month: int) (day: int) (hour: int) (minute: int) (second: int) : int = 
  if year < 1970 then 0
  else begin
    let tot_days = ref 0 in
    (* Account for the year part. *)
    for y = 1970i to (Z.to_int (year - 1)) do 
      if (is_leap_year (Z.of_int y)) then tot_days := !tot_days + 366 else tot_days := !tot_days + 365
    done;
    (* Account for the month part. *)
    tot_days := !tot_days + yeardays_months.(Z.to_int @@ month - 1) + day - 1;
    (* And for leap years. *)
    if (is_leap_year year) && month > 2 then tot_days := !tot_days + 1;
    !tot_days * 86400 + hour * 3600 + 
       minute * 60 + second
  end
